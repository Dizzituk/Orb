...
old = '''def _is_host_only(path: str) -> bool:
    ...
    return False
'''
new = '''def _is_host_only(path: str) -> bool:
    """Check if path is host-only data (architecture maps, logs)."""
    for prefix in _HOST_ONLY_PREFIXES:
        if path.startswith(prefix):
            return True
...
'''
text = text.replace(old, new)
old = '''async def execute_write_file(params: Dict[str, Any]) -> str:
...
'''
new = '''async def execute_write_file(params: Dict[str, Any]) -> str:
...
'''
text = text.replace(old, new)
old = '''async def execute_run_command(params: Dict[str, Any]) -> str:
    ...
    """
'''
new = '''async def execute_run_command(params: Dict[str, Any]) -> str:
...
old = '''    try:
        ...
        return result
'''
new = '''    protected_cwd = _resolve_sandbox_path(cwd)
...
print("repatched action_executor.py")
