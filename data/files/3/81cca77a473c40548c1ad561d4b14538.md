...
**Version:** 0.12.18 (Architecture Map v24)  
...
- **High-stakes critique pipeline: Opus → Gemini 3 critique → Opus revision** (v0.12.18)
...
**Note:** `"orchestrator"` is intentionally **excluded** (too generic, would over-trigger)
...
```python
HIGH_STAKES_JOB_TYPES = { ... }  # 11 specific job types
...
should_run_critique = (
    ...
    is_high_stakes_job(classified_type.value)  # ← STRING check, not enum
)
...
if should_run_critique:
    return await run_high_stakes_with_critique(
        ...
        model_id=model_id,
        envelope=envelope,
        job_type_str=classified_type.value,  # ← Pass STRING job type
    )
```
...
All critique pipeline actions log with `[critic]` prefix:

```python
logger.info("[critic] High-stakes pipeline enabled: job_type=%s model=%s", ...)
logger.info("[critic] Calling Gemini 3 Pro for critique of {job_type} task")
...
GEMINI_OPUS_CRITIC_MODEL=gemini-3-pro

# Minimum response length in characters to trigger critique (~250-300 tokens)
...
2. Check model name is `gemini-3-pro` (not `gemini-3.0-pro-preview`)
3. Verify Google API quota not exceeded
4. Test network connectivity to Google AI
...
**High-Stakes Critique Pipeline (v0.12.18):**
...
- [ ] `"orchestrator"` does NOT trigger critique (excluded from set)
