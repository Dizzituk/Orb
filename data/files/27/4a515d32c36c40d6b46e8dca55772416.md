...
**Version:** 0.15.2 (Architecture Map v31)
...
- Local web search via Tool Registry (`web_search@v1` via DuckDuckGo HTML/Lite); provider-side browsing/grounding disabled
...
```text
D:/
├── Orb/                               # Python backend (FastAPI)
│   ├── main.py                        # FastAPI app entrypoint (authoritative)
│   ├── app/
...
│   │   │   ├── streaming.py           # provider streaming adapters
│   │   │   ├── token_budgeting.py
│   │   │   ├── web_search.py          # tool-driven search + optional answer pass
│   │   │   └── web_search_router.py   # /web_search endpoint + health
│   │   ├── tools/                     # Local tools (ONLY place with outbound HTTP)
│   │   │   ├── executor.py            # safety/timeout/validation + implementations
│   │   │   ├── registry.py            # tool registry + execute_tool_async
│   │   │   └── schemas.py             # tool I/O schemas (jsonschema-validated)
│   │   ├── providers/
│   │   │   └── registry.py            # OpenAI/Anthropic/Google clients + routing
│   │   ├── jobs/
│   │   ├── memory/
│   │   ├── crypto/
│   │   └── ...
│   ├── data/                          # Local DB + files (encrypted-at-rest where applicable)
│   │   ├── db/
│   │   └── files/
│   └── static/                        # (legacy/aux) contains a shadow main.py; NOT the entrypoint
├── orb-desktop/                        # Electron frontend (runs backend + UI)
│   └── ...
└── Windows Credential Manager
    └── OrbMasterKey/default           # Master key (URL-safe base64)
```
...
GEMINI_VIDEO_DEEP_MODEL=gemini-3-pro-preview
...
| Gemini (video deep) | `GEMINI_VIDEO_DEEP_MODEL` | `gemini-3-pro-preview` |
...
| `VIDEO_HEAVY` | Google | `gemini-3-pro-preview` | ALL videos (v0.14.1: no more Flash) |
...
| `CROSS_MODEL_REVIEW` | Google | `gemini-3-pro-preview` | Cross-model critique/review |
...
2. Analyze attachments:
   - Video present? → Check size AND semantic keywords (v0.12.8)
     - >10MB OR needs_deep_analysis → HEAVY_MULTIMODAL_CRITIQUE (gemini-3-pro-preview)
...
if total_size > VIDEO_SIZE_THRESHOLD:
    → VIDEO_HEAVY (gemini-3-pro-preview)
...
**UX Recommendations (Frontend):**

The 45-60s pipeline delay requires careful UX handling to prevent users thinking the app crashed:

```typescript
// Suggested frontend implementation
const HIGH_STAKES_TIMEOUT = 90000; // 90s before showing warning

// Show progressive status indicators:
// 0-15s:  "Analyzing request..."
// 15-30s: "Generating draft..."
// 30-45s: "Reviewing for quality..."
// 45-60s: "Finalizing response..."
// 60s+:   "This is taking longer than usual..." (with cancel option)
...
**Note:** Future enhancement could send SSE progress events from backend during pipeline stages.
...
**Purpose:** Real‑time assistant replies using Server‑Sent Events (SSE).

**Key cleanup in this version:** Streaming logic lives in **two places only**:
- **`app/llm/stream_router.py`** → FastAPI SSE endpoints + SSE serialization (`data: {json}

`)
- **`app/llm/streaming.py`** → Provider streaming generators (OpenAI / Anthropic / Gemini)

### File Locations

| Component | File | Notes |
|---|---|---|
| SSE endpoints | `app/llm/stream_router.py` | `POST /stream/chat`, `GET /stream/providers` |
| Provider streaming | `app/llm/streaming.py` | `stream_llm()` + `stream_openai()` / `stream_anthropic()` / `stream_gemini()` |
| Routing + critique pipeline | `app/llm/router.py` | High‑stakes orchestration + critique pipeline |
| Job classification | `app/llm/job_classifier.py` | Determines job type / provider / model (and OVERRIDE detection) |

### Streaming Event Contract

**Provider generators now yield Python dict events (NOT JSON strings):**
...
**SSE layer (`stream_router.py`) remains the only place that JSON‑serializes events.**  
For UI backward‑compatibility, `stream_router.py` currently re-emits `token` events as `{"type":"token","content":"..."}`.

### Date/Time Context (System Prompt)

Date/time context is injected in `app/llm/streaming.py` via `enhance_system_prompt()` (and optionally the reasoning instruction via `enhance_system_prompt_with_reasoning()`).

### Known Drift / Edge Cases (observed in code)

- `stream_router.py` calls `stream_llm()` without forwarding `enable_reasoning`, so provider‑side “reasoning tag” instructions may not activate unless you wire that flag through.
- `stream_router.py` defensively normalizes legacy string tokens to dict events to avoid `AttributeError: 'str' object has no attribute 'get'`.
...
**Goal:** deterministic, local, provider-agnostic web search that never enables provider-side browsing/grounding.

### Current implementation (v31)

**FastAPI endpoints**
- `app/llm/web_search_router.py`
  - `POST /web_search` → calls `search_and_answer()`
  - `GET /web_search/health` → health check

**Search + answer orchestration**
- `app/llm/web_search.py`
  1) Calls the local tool layer: `execute_tool_async("web_search", "v1", {"query","max_results"})`
  2) Builds an evidence block from `{title, url, snippet}` sources
  3) Optionally calls an LLM to answer **using ONLY those sources**, with citations like `[1]` and with `enable_web_search=False`

**Local tool layer (the only place that touches the internet)**
- `app/tools/registry.py` — tool registration + `execute_tool_async(...)`
- `app/tools/executor.py` — tool execution + safety (timeouts, size limits, allow/deny lists, schema validation)
- `app/tools/schemas.py` — JSON schemas for all tool inputs/outputs (validated via `jsonschema`)

**Tool: `web_search@v1`**
- Providers: `duckduckgo_html` (primary) with `duckduckgo_lite` fallback
- Output: `{"provider": "...", "results": [{"title","url","snippet"}...]}` (snippet-only; no full-page fetch)

### Safety / correctness guarantees

- **No provider-side browsing:** LLM calls are forced to `enable_web_search=False` and answers are constrained to tool sources.
- **Network access is centralized:** all outbound HTTP is inside the tool executor (easy to audit / lock down).
- **Schema-driven I/O:** tool inputs/outputs are validated against `app/tools/schemas.py` (requires `jsonschema` in backend venv).

### Known limitations (tracked for upgrade)

- DDG HTML can include sponsored/ad redirect URLs; add a filter/dedupe pass (prefer canonical domains).
- Snippet-only evidence can miss exact numeric tables; add an optional second pass:
  - `http_fetch@v1` on the top N canonical URLs + lightweight extraction, then re-answer.
- Search results can include *predictions* (e.g., Reuters polls) vs *official decisions*; add domain ranking and “official-first” selection.

### Status vs Plateau 1 “Internet Access & Tool Binding”

✅ General tool registry + schema layer exists  
✅ All web access goes through Orb tools (no provider browsing)  
✅ Safety/timeout/validation layer exists in `executor.py`  
⚠️ “Provider tool-call bindings” are not exposed to models as native tool calls yet; backend calls tools first, then injects results as context
## Chat History
...
4. Response
   - Returns: {answer, provider: "google", model: "gemini-3-pro-preview"}
...
**Expected Limits by Provider:**

| Provider | RPM (Requests/Min) | TPM (Tokens/Min) | Notes |
|----------|-------------------|------------------|-------|
| OpenAI (GPT-4.1) | 500 | 30,000 | Tier 1 defaults |
| Anthropic (Claude) | 50 | 40,000 | Varies by plan |
| Google (Gemini 3 Pro) | 15 | 32,000 | Preview model, stricter limits |
| Google (Gemini 2.5 Pro) | 60 | 120,000 | Production model |

**Retry Strategy:**

```python
# Implemented in router.py / fallbacks.py
MAX_RETRIES = 3
BACKOFF_BASE = 2  # seconds
JITTER_MAX = 1    # seconds

# On 429 (Rate Limited):
# - Wait: BACKOFF_BASE^attempt + random(0, JITTER_MAX)
# - Retry up to MAX_RETRIES times
# - Then fallback to next model in chain

# On 5xx (Server Error):
# - Immediate retry once
# - Then backoff strategy
# - Then fallback
```

**User-Facing Behavior When Quota Exhausted:**

1. First fallback model attempted automatically
2. If all fallbacks fail: Return error with message:
   - "Service temporarily unavailable. Please try again in a few minutes."
3. Logs show: `[fallback] All models exhausted, returning error`

**Monitoring:**
- Check `ORB_ROUTER_DEBUG=1` logs for rate limit hits
- Look for: `[fallback] MODEL_RATE_LIMITED, waiting Xs before retry`
...
- [~] Tool integration: **limited** provider-side web search endpoint exists (Gemini grounding). General **local tool registry + safety/timeouts + provider tool-call bindings** still pending.
...
**Feature-flagged:** Yes (`ORB_ENABLE_PHASE4=true`)  
**Target version:** TBD
...
Key architectural decisions are documented inline in code comments. For significant decisions, consider maintaining a `DECISIONS.md` file:

```markdown
# DECISIONS.md (Example)

## 2024-12: Cross-Model Critique Pipeline
- Why Gemini for critique instead of a second Claude call?
- Reduces single-provider dependency; catches Claude-specific blind spots
- Cost: ~2.7x overhead, but quality improvement justifies for high-stakes

## 2024-12: SQLite over Postgres
- Single-host deployment; no need for network database
- Simpler backup (copy file), no Docker dependency
- Encryption via Fernet at field level

## 2024-12: Windows Credential Manager for Master Key
- Leverages OS-level security without external dependencies
- No plaintext keys in config files or environment variables
- Appropriate for local desktop app security model
```
...
- **v31 (Dec 14, 2025)**  
  - Web search is **local-tools-first**: DuckDuckGo HTML/Lite via `web_search@v1` (no provider browsing).
  - Tool Registry stack is present and wired: `app/tools/{registry,executor,schemas}.py` (centralized safety + schema validation).
  - Web search orchestration (`app/llm/web_search.py`) can produce a cited answer using only tool sources.
  - Backend dependency: `jsonschema` required for schema validation in the tool executor.

- **v30 (Dec 13, 2025)**  
  - Web search split is clean: `web_search_router.py` (HTTP) separate from `stream_router.py` (SSE).
  - Web search orchestration consolidated in `web_search.py` (tool-driven search + optional answer pass).
  - Streaming provider generators standardized to dict events in `streaming.py`; `stream_router.py` is sole JSON/SSE serializer.
  - Noted drift: `enable_reasoning` is not currently forwarded from `stream_router.py` into `stream_llm()`.
...
- `GEMINI_OPUS_CRITIC_MODEL=gemini-3-pro-preview` (Gemini critic model)
...
- Added `GEMINI_VIDEO_DEEP_MODEL` env var (default: gemini-3-pro-preview)
