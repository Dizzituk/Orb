...
- Auth setup and validation endpoints
...
from app.auth import require_auth, is_auth_configured, get_api_key
...
def on_startup():
    ...
    if is_auth_configured():
        print("[startup] API authentication: ✓ configured")
        key = get_api_key()
        if key:
            print(f"[startup] API key: {key[:8]}...{key[-4:]}")
    else:
        print("[startup] API authentication: ✗ NOT CONFIGURED")
        print("[startup] Call POST /auth/setup to generate an API key")
    
    # Verify critical env vars
    print("[startup] Checking environment variables...")
    if os.getenv("GOOGLE_API_KEY"):
        print("[startup] GOOGLE_API_KEY: ✓ set")
    else:
        print("[startup] GOOGLE_API_KEY: ✗ NOT SET - image analysis will fail")
...
# Memory router - will be protected
