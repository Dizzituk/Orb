...
def _extract_search_queries(text: str, category: TopicCategory) -> list[str]:
    ...
    Strips conversational filler and extracts the factual core.
    """
    ...
    # For claims: the cleaned message IS the search query (often)
    queries = [cleaned]
    
    # If it's long, also try a shorter extraction
    words = cleaned.split()
    if len(words) > 8:
        # Take the noun-heavy tail (skip leading verbs)
        short = " ".join(words[-6:])
        if short != cleaned:
...
def classify_topic(text: str) -> ClassificationResult:
    ...
    Priority order:
    1. PERSONAL signals checked first (short-circuit for greetings, system queries)
    ...
    Returns ClassificationResult with category, confidence, and metadata.
    """
    ...
    # 1. Strong personal signal with NO claims/contested → personal
    if p_count > 0 and c_count == 0 and x_count == 0:
        return ClassificationResult(
            category=TopicCategory.PERSONAL,
    ...
    #    Contested topics ALSO need grounding, but with multi-perspective search
    if x_count > 0:
        queries = _extract_search_queries(text_clean, TopicCategory.CONTESTED)
        confidence = min(1.0, 0.6 + (x_count * 0.15))
        # If also has claims signals, boost confidence
        if c_count > 0:
    ...
    #    Catches things like "how's the economy doing" that don't hit keywords
    ...
    if _QUESTION_ABOUT_WORLD.match(text_clean):
        # Check if it's about the external world (not about the user/system)
        ...
        if not is_about_self and len(text_clean.split()) >= 5:
            ...
            return ClassificationResult(
                category=TopicCategory.CLAIMS,
                confidence=0.5,  # Low confidence — heuristic match
