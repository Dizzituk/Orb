...
def get_capability_context() -> str:
    ...
    if sandbox.get('allowed', True):
        ...
        lines.append("")
        lines.append("   **Code Repositories (read + write in sandbox):**")
        lines.append("   - Backend repo:  D:\\Orb (Python/FastAPI)")
        lines.append("   - Frontend repo: D:\\orb-desktop (React/TypeScript/Vite)")
        lines.append("   - Android repo:  D:\\Astra Android Folder\\Astra-Bridge (Kotlin/Android Studio)")
        lines.append("")
        lines.append("   **User Directories (read access via Drive API at /drive/*):**")
        lines.append("   - Documents:    User home\\Documents")
        lines.append("   - Pictures:     User home\\Pictures")
        lines.append("   - Screenshots:  User home\\Pictures\\Screenshots")
        lines.append("   - Music:        User home\\Music")
        lines.append("   - Videos:       User home\\Videos")
        lines.append("   - Desktop:      User home\\Desktop")
        lines.append("   - ASTRA output: D:\\Orb\\output")
        lines.append("")
        lines.append("   **Access Rules:**")
        lines.append("   - You CAN: read files, write files (sandbox only), execute code, explore directories")
        lines.append("   - Read access is BROAD: repos, user folders, Android project — use it to understand context")
        lines.append("   - Write access is SANDBOXED: only write inside the sandbox environment")
