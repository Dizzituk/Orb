...
def _build_initial_prompt(
    ...
    parts.append("1. Start with backend models, schemas, services, routers (CREATE files)")
    parts.append("2. Write frontend components (CREATE files)")
    parts.append("3. CRITICAL — Wire integration points BEFORE you run out of budget:")
    parts.append("   - MODIFY files are the glue that makes the feature visible")
    parts.append("   - JobPage.tsx routing, main.py router registration, CSS imports")
    parts.append("   - These are small targeted changes but without them NOTHING renders")
    parts.append("4. Boot and verify")
    parts.append("")
    parts.append("DO NOT leave MODIFY files (integration wiring) until last.")
    parts.append("If you write all CREATE files but skip the MODIFY wiring,")
    parts.append("the feature will be invisible even though the code exists.")
