# Purpose: simple create utils 14
# Called-by: app.pot_spec.grounded._simple_create_evidence, app.pot_spec.grounded._simple_create_utils_15, app.pot_spec.grounded._simple_create_utils_17, app.pot_spec.grounded.simple_create
# Depends-on: app.pot_spec.grounded._sbx_fs, app.pot_spec.grounded._simple_create_utils_13, app.pot_spec.grounded._simple_create_utils_17
...
def _suggest_new_files(
    ...
    for path in project_paths:
        ...
        if 'desktop' in path_lower or 'frontend' in path_lower or 'ui' in path_lower:
            frontend_path = path
        elif _sbx_exists(os.path.join(path, 'requirements.txt')):
            backend_path = path
        elif _sbx_exists(os.path.join(path, 'package.json')):
            frontend_path = path
        elif _sbx_exists(os.path.join(path, 'app')):
            backend_path = path
...
def _host_list_directory(dir_path: str, max_entries: int = 200, project_paths: Optional[List[str]] = None) -> Tuple[bool, str]:
    ...
    if not _sbx_exists(dir_path) and project_paths:
    ...
    if not _sbx_exists(dir_path):
        return False, f"Directory not found: {dir_path}"
    if not _sbx_isdir(dir_path):
