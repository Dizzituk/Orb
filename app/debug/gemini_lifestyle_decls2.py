...
LIFESTYLE_TOOL_DECLARATIONS_MORE: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            "Look up a BRANDED / packaged / restaurant food's nutrition ONLINE by "
            "its product name or barcode and log it. USE THIS ONLY for a named "
            "manufactured product or a barcode — e.g. 'Aldi Chicken, Tomato & "
            "Basil Topped Pasta', a named ready meal, a specific protein bar, a "
            "named takeaway dish. It runs a ladder: your local product library → "
            "Open Food Facts (barcode or name) → a retailer product page you pass "
            "in `url` → a SIMILAR same-type item (flagged as an estimate) → and "
            "only if nothing fits does it ask you to get the macros. It logs ONE "
            "row with the right portion, source and micronutrients.\n"
            "\n"
            "DO NOT use this for GENERIC whole foods — fruit, veg, plain meat or "
            "fish, plain yogurt, whey/protein powder, eggs, nuts, oils, rice, "
            "pasta. Open Food Facts will return an unrelated branded product with "
            "the wrong macros (it returned 'Chinese Style Pork Loin Steaks' for "
            "plain pork, and cereal/protein BARS for plain fruit and whey). For "
            "generic whole foods call `log_nutrition` instead, with your OWN "
            "per-item macro estimate (ask the user or web_search if unsure).\n"
            "\n"
