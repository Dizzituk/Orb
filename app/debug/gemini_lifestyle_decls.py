# FILE: app/debug/gemini_lifestyle_decls.py
...
LIFESTYLE_TOOL_DECLARATIONS: List[Dict[str, Any]] = [
    {
        ...
        "description": (
            ...
            "\n"
            "DON'T RE-LOG — before logging, check what's already in today's "
            "diary (get_today_summary / get_recent_nutrition). If the user says "
            "'log it' / 'log everything' again and those foods are already logged "
            "today, do NOT log them a second time — just confirm what's there. "
            "Re-running a whole log pass on each request is the main cause of "
            "duplicate diary rows.\n"
            "\n"
            "ITEMISE \u2014 when the user ate several distinct foods in a meal "
            "(e.g. 'chicken, rice and broccoli'), pass them as the `items` "
            "array: ONE object per food, each with its OWN description and its "
            "OWN calories, protein_g, carbs_g, fat_g (and sugar_g where known). "
            "NEVER merge several foods into one combined entry with summed "
            "macros -- the tool REJECTS a single description that lists multiple "
            "foods. You MUST itemise, e.g. '100g blueberries, 130g strawberries, "
            "250g Greek yoghurt, 480g pork loin' -> 4 items; 'Aldi pasta; fish "
            "sushi; 400g strawberries; ice cream pot' -> 4 items. A single "
            "product whose NAME contains a comma is still ONE item (e.g. 'Aldi "
            "Chicken, Tomato & Basil Topped Pasta' = one item). Use the single "
            "description+macros form ONLY for a genuinely single food (e.g. one "
            "Snickers bar). meal_type applies to the whole call.\n"
            "\n"
