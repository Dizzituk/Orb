...
1. **Target 20 KB / 500 lines per file. Hard cap 30 KB for logic-heavy files.** Exception: data-heavy files (constants, lookup tables, prompt templates, configuration, schema definitions) may exceed 30 KB if their logic portion is small. The cap applies to logic, not data.
2. **One public function per module** (preferred). Each file should expose at most one public function or one closely-related group of private helpers. Multiple small related functions in one file are acceptable if they share a single responsibility.
...
5. **Pre-flight size estimation:** Before finalising the File Inventory, estimate the line count of each planned file. If any file would exceed 600 lines of logic, split it NOW — do not leave it to the implementer.
...
Rationale: The implementer LLM generates files in a single pass. Files exceeding ~800 lines risk truncation, causing build failures. Smaller files also improve RAG retrieval precision and simplify future refactoring.
""")
