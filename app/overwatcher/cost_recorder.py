...
Usage (in _registry_utils_3.py):
    from app.overwatcher.cost_recorder import record_llm_cost, pre_call_budget_check
...
def record_llm_cost(
    ...
    """
    try:
        from app.overwatcher.cost_pricing import calculate_cost
        from app.overwatcher.cost_ledger import append_record
        from app.overwatcher.cost_guard import record_usage, ModelRole
...
def pre_call_budget_check(
    ...
    """
    try:
        from app.overwatcher.cost_budget import is_call_allowed
...
def _stage_to_role(stage: Optional[str]) -> "ModelRole":
    ...
    from app.overwatcher.cost_guard import ModelRole
