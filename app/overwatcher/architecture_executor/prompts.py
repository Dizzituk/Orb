...
3. Follow the modification specification exactly.
4. Preserve all code NOT mentioned in the modification spec.
5. If the spec says "add", insert the new code at the appropriate location.
6. If the spec says "modify", change only what's specified.
7. If the spec says "remove", delete only what's specified.
8. Maintain the file's existing style, imports, and structure.
9. CROSS-FILE REFERENCES: If consuming code or other files reference symbols in this file, ensure those symbols still exist after modification.
10. DO NOT remove imports, classes, or functions unless the spec explicitly says to.
11. NO EMOJI: Do NOT use emoji characters (Unicode above U+FFFF) anywhere in Python source code — not in strings, print statements, comments, docstrings, or log messages. Use plain ASCII text only.
"""
